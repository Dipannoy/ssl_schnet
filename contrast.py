...
class CrystalContrastive(object):
    def __init__(self, config):
        ...
        dir_name = 'schnet_pretrain_'+current_time
            ...
            for bn, (input_1, input_2, _) in enumerate(self.train_loader):
                ...
                if self.config['cuda']:
                    input_var_rot_1 = (Variable(input_1[0].to(self.device, non_blocking=True)),
                                ...
                                Variable(input_1[2].to(self.device, non_blocking=True)),
                                input_1[3].to(self.device, non_blocking=True),
                                [crys_idx.to(self.device, non_blocking=True) for crys_idx in input_1[4]])

                    input_var_rot_2 = (Variable(input_2[0].to(self.device, non_blocking=True)),
                                Variable(input_2[1].to(self.device, non_blocking=True)).float(),
